...
export const Privacy = observer(({
  ...
  const getData = async () => {
    ...
    await fetch(REACT_APP_API_URL + 'privacy', {
      headers: {
        method: 'GET',
  ...
  return (
    <>
      ...
      {
        handleAgree &&
        <Stack direction={'row'} sx={{ position: 'fixed', bottom: 0, left: 0, right: 0 }}>
          <Button variant={'contained'} color={'inherit'} size={'large'} sx={{ flex: .5, borderRadius: 0 }} onClick={handleClose}>취소</Button>
