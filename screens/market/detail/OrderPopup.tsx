...
export const OrderPopup = observer(
  ({ handleClosePopup, isFirst, goodsCheck, currencyCd }: Props) => {
    ...
    const [goods, setGoods] = useState<IGoodsModel | undefined>();

    useEffect(() => {
      setShowToolbar(true);
      // 현재상품
      const _goodsSid = parseInt(goodsSid || '0');
      const item = goodsStore?.list?.find((v) => v.goodsSid === _goodsSid);
      setGoods(item);
    ...
    const handleBuy = async () => {
      ...
      if (goodsCheck.alertYn) {
        setIsAlertOpen(goodsCheck.alertYn);
        setAlertMsg(goodsCheck.alertMsg!);
        setIsSuccess(true);
      } else {
        if (isSuggestion) {
          navigate('/market/payment/', { state: { goods: getSnapshot(orderStore.goods) } });
        } else {
          orderStore.setProps({
            goodsSid: 0,
          ...
          goods && navigate('/market/payment/', { state: { goods: getSnapshot(goods) } });
        }
      }
    };
          ...
          >
            ...
            >
              <Stack sx={{ direction: 'row', width: '100%' }}>
                <Box
                  sx={{ alignSelf: 'flex-end', mt: pxToRem(20), cursor: 'pointer' }}
                  onClick={() => {
                    handleClosePopup();
                  }}
                >
                ...
                <Stack sx={{ mb: pxToRem(40) }}>
                  ...
                  <Typography
                    variant="Kor_14_r"
                    sx={{ my: pxToRem(20), textAlign: 'center', lineHeight: pxToRem(22) }}
                  >
                    고객님의 누적 구매 예정 금액은{' '}
                    <span style={{ fontWeight: 600, color: theme.palette.primary.main }}>
                      {numberComma(goodsCheck.estimatedAccumPaymentAmt)}
                      {currencyCd}
                    </span>{' '}
                    입니다.
                    <br />
                    ...
                    <span style={{ fontWeight: 600 }}>
                      {numberComma(goodsCheck.goodsAmt || 0)}
                      {currencyCd}
                    </span>
                    에 구매하실 수 있습니다. (키트비 제외)
                  </Typography>
                  {goodsCheck.goodsSid != null && (
                    <Button
                      id={'btn-market-package-buy-ok'}
                      sx={{
                        width: 335,
                        height: 50,
                        mb: pxToRem(20),
                  ...
                  <Typography
                    id={`btn-market-package-buy-cancel`}
                    sx={{
                      fontSize: pxToRem(12),
