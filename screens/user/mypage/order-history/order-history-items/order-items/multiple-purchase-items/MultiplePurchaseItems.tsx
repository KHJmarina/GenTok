...
import { OrderHistoryStore } from 'src/models';
...
import OpenStatusItems from './open-status-items/OpenStatusItems';
import CloseStatusItems from './open-status-items/CloseStatusItems';
/**
...
export const MultiplePurchaseItems = observer(({ data }: Props) => {
  ...
  return (
    <>
      {isOpen == false ? (
        <CloseStatusItems data={data} isOpen={isOpen} handleClick={handleClick}/>
      ) : <OpenStatusItems data={data} isOpen={isOpen} handleClick={handleClick} /> }
    </>
  );
});
export default MultiplePurchaseItems;
